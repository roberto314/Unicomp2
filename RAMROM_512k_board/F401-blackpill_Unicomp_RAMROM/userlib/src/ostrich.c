...
static THD_FUNCTION(CharacterInputThread, arg) {
  ...
  while (true){
    ...
    if (1){
      ...
      end = chTimeAddX(start, TIME_MS2I(300));
...
//        break;
      case CLOCK_DWnCs:
        ...
        if (c == cs){
          if (DEBUGLEVEL == 2){
            chprintf(dbg, "Clock (Wr): %02X, %02X, %02X, %02X, %02X, %02X, %02X, %02X, %02X\r\n", clkbuf[0], clkbuf[1], clkbuf[2], clkbuf[3], clkbuf[4], clkbuf[5], clkbuf[6], clkbuf[7], clkbuf[8]);
          }
          write_clock(&clkbuf[0]);
          chprintf(ost, "O");
        }
        else{
          chprintf(dbg, "Checksum ERROR\r\n");
      ...
      case CLOCK_DRCs:
        ...
        state = IDLE;
        if (c == cs){
          count = (uint16_t)read_clock(&clkbuf[0]);
          temp=0;
          for (i=0;i<count;i++){
            if (DEBUGLEVEL == 2){
              chprintf(dbg, "%i, %02x\r\n", i, clkbuf[i]);
            }
            streamPut(ost, clkbuf[i]);
            temp += clkbuf[i];
          }
          if (DEBUGLEVEL == 2){
            chprintf(dbg, "CS: %02x\r\n", (uint8_t)temp);
          }
          streamPut(ost, (uint8_t)temp);
        }
        else{
          chprintf(dbg, "Checksum ERROR\r\n");
        }
